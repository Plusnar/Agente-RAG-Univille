...
from config import (
    ...
    EMBEDDING_PROVIDER,
    EMBEDDING_RATE_LIMIT_RETRY_SECONDS,
...
RATE_LIMIT_MESSAGE = (
    ...
    "Para chave trial, mantenha EMBEDDING_TOKENS_PER_MINUTE abaixo do limite "
    "da sua conta, por exemplo 90000 para o limite de 100000 tokens/min."
)
TOKEN_RATE_WINDOW_SECONDS = 60
APPROX_CHARS_PER_TOKEN = 3
TOKEN_OVERHEAD_PER_TEXT = 12
...
def _estimate_embedding_tokens(text: str) -> int:
    ...
    return max(1, (compact_length + APPROX_CHARS_PER_TOKEN - 1) // APPROX_CHARS_PER_TOKEN) + TOKEN_OVERHEAD_PER_TEXT


def _estimate_batch_tokens(batch: List[Document]) -> int:
    return sum(_estimate_embedding_tokens(chunk.page_content) for chunk in batch)
...
def _embedding_batches(
    ...
    current_tokens = 0
    token_limit = max(0, max_tokens_per_batch)

    for chunk in chunks:
        chunk_tokens = _estimate_embedding_tokens(chunk.page_content)
        would_exceed_size = len(current_batch) >= max_batch_size
        would_exceed_tokens = (
            token_limit > 0
            and current_batch
            and current_tokens + chunk_tokens > token_limit
        )
...
class TokenRateLimiter:
    ...
    def wait_for_capacity(self, tokens: int) -> None:
        ...
        tokens = min(max(1, tokens), self.tokens_per_minute)

        while True:
            now = time.monotonic()
            self._discard_expired(now)
            used_tokens = sum(event_tokens for _, event_tokens in self._events)
            ...
            wait_seconds = max(1, TOKEN_RATE_WINDOW_SECONDS - (now - oldest_timestamp) + 1)
            if self.progress:
                self.progress(
                    "Limite de tokens/minuto: aguardando "
                    f"{wait_seconds:.0f}s para continuar sem pular chunks..."
                )
    ...
    def _discard_expired(self, now: float) -> None:
        while self._events and now - self._events[0][0] >= TOKEN_RATE_WINDOW_SECONDS:
...
def _add_documents_with_backoff(
    ...
    retry_sleep_seconds = 0 if EMBEDDING_PROVIDER == "local" else max(1, EMBEDDING_RATE_LIMIT_RETRY_SECONDS)
    tokens_per_minute = 0 if EMBEDDING_PROVIDER == "local" else EMBEDDING_TOKENS_PER_MINUTE
    rate_limiter = TokenRateLimiter(tokens_per_minute, progress=progress)
    batches = _embedding_batches(chunks, batch_size, tokens_per_minute)
    total_batches = len(batches)
    processed_chunks = 0

    for batch_number, (batch, estimated_tokens) in enumerate(batches, start=1):
        ids = [chunk.metadata["chunk_id"] for chunk in batch]
        next_processed_chunks = processed_chunks + len(batch)
        progress_percent = min(100, round((next_processed_chunks / total) * 100, 1))

        if progress:
            progress(
                f"Gerando embeddings lote {batch_number}/{total_batches} "
                f"({next_processed_chunks}/{total} chunks, {progress_percent}%, "
        ...
        processed_chunks = next_processed_chunks

        if batch_number < total_batches and sleep_seconds > 0:
            if progress:
