...
from config import ADMIN_PASSWORD, ADMIN_USERNAME, DATA_DIR, IS_VERCEL, ensure_directories
...
def _check_writable() -> None:
    if IS_VERCEL:
        raise HTTPException(
            status_code=503,
            detail="Upload e indexacao estao desativados em producao. Atualize os documentos localmente e faca redeploy.",
        )
...
def health() -> dict[str, str]:
    from config import EMBEDDING_PROVIDER, LLM_PROVIDER, prepare_chroma_runtime

    chroma_path = prepare_chroma_runtime() if IS_VERCEL else None
    ...
    return {
        ...
        "chroma_dir": str(chroma_path) if chroma_path else "local",
    }
